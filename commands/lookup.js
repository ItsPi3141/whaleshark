				...
				fetch(`https://api.duckduckgo.com/?q=${args.join(" ")}&format=json&skip_disambig=1`).then((res) => {
					res.json().then((json) => {
						if (json.AbstractText) {
							reply.edit({
								content: "",
								embeds: [new EmbedBuilder().setTitle("Search result for " + args.join(" ")).setDescription(json.AbstractText + `\n\n*Read more on [Wikipedia](${json.AbstractURL})*`)]
							});
						} else {
										...
										reply.edit({
											...
											embeds: [
												new EmbedBuilder()
													...
													.setDescription(
														"### Definition\n" +
															ubjson.list[0].definition.replaceAll("[", "").replaceAll("]", "") +
															"\n\n### Usage\n" +
															ubjson.list[0].example.replaceAll("[", "").replaceAll("]", "") +
															`\n\n*Read more on [Urban Dictionary](${ubjson.list[0].permalink})*`
													)
													.setFooter({
														text: (ubjson.list[0].author
															? [ubjson.list[0].word, ubjson.list[0].author, "👍 " + ubjson.list[0].thumbs_up, "👎 " + ubjson.list[0].thumbs_down]
