	...
	execute(message, args) {
		if (args.length === 0) {
			message.reply({
				embeds: [
					new EmbedBuilder().setDescription(
						"Search something up on the internet\n```\nws lookup <query>\n```",
					),
				],
			});
		} else {
			message.reply(`${config.emojis.loading} Searching...`).then((reply) => {
				try {
					fetch(
						`https://api.duckduckgo.com/?q=${encodeURIComponent(args.join(" "))}&format=json&skip_disambig=1`,
					).then((res) => {
						res.json().then((json) => {
							if (json.AbstractText) {
								...
								fetch(
									`https://api.urbandictionary.com/v0/define?term=${encodeURIComponent(args.join(" "))}`,
								).then((ubres) => {
